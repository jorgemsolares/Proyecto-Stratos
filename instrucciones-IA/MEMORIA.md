...
1. **Al abrir una sesión nueva**, la IA lee este archivo completo + `instrucciones.md`.
...
_(Se irán agregando más notas aquí conforme el Arquitecto las comparta.)_
...
- **PASO 3:** Mover de `datos.js` → `logica.js`: `reconstruccionOrganigramaDesdeUsuario`.
- **PASO 4:** Mover de `datos.js` → `logica.js` las 7 funciones de infraestructura (sin borrar lógica).
- **PASO 5:** Verificación final global (orden de carga, sin duplicados, sin refs muertas).
...
- **`datos.js` — corregida la CODIFICACIÓN de los títulos/comentarios:**
  - El archivo tenía **doble-codificación** (UTF-8 ↔ cp1252): tildes, Ñ y emojis salían corruptos (`SesiÃ³n`, `CONFIGURACIÃ“N`).
